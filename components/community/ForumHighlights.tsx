        ...
        {TOPICS.map((post: CommunityPost) => (
          <div key={post.id} className="p-4 rounded-lg bg-gray-50 dark:bg-gray-700/50 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors">
            <div className="flex items-start">
              <img src={post.author.avatar} alt={post.author.name} className="h-10 w-10 rounded-full mr-4" />
              <div className="flex-grow">
                ...
                <h3 className="font-semibold text-lg text-gray-800 dark:text-gray-200 mt-1 cursor-pointer hover:text-green-600">{post.title}</h3>
                <div className="flex items-center text-sm text-gray-500 dark:text-gray-400 mt-2">
                  <p>by <span className="font-semibold">{post.author.name}</span></p>
                  ...
                  <span className="mx-2">&bull;</span>
                  <p>Last reply {post.lastActivity}</p>
                </div>
              </div>
            </div>
          </div>
