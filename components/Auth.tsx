...
export const Auth: React.FC<AuthProps> = ({ onLogin, onNavigate }) => {
  ...
  useEffect(() => {
    const handleRedirectResult = async () => {
      const loadingProvider = sessionStorage.getItem('authProvider');
      if (loadingProvider) {
          setAuthProvider(loadingProvider);
          switch (loadingProvider) {
              case 'google': setIsGoogleLoading(true); break;
              case 'github': setIsGithubLoading(true); break;
              case 'microsoft': setIsMicrosoftLoading(true); break;
          }
      }
        ...
        if (result) {
          ...
          if (!userDoc.exists()) {
            userData = {
              ...
              role: 'Farmer',
              esin: `${esinPrefix}-${new Date().getFullYear()}-${Math.floor(1000 + Math.random() * 9000)}`,
  ...
  const handleOAuthLogin = async (providerName: 'google' | 'github' | 'microsoft') => {
    setError(null);
    let provider;
    sessionStorage.setItem('authProvider', providerName);
        ...
        <div className="fixed inset-0 z-[200] bg-black/60 backdrop-blur-md flex items-center justify-center p-4 animate-in fade-in duration-300">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-300">
             <div className="p-8 text-center">
                <Loader2 className={`animate-spin ${authProvider === 'google' ? 'text-blue-600' : authProvider === 'github' ? 'text-slate-800' : 'text-blue-500'} mx-auto mb-4`} size={48} />
                <h3 className="text-xl font-bold text-slate-800">Authenticating with {authProvider?.charAt(0).toUpperCase() + authProvider!.slice(1)}...</h3>
             </div>
          </div>
        </div>
      )}
          ...
          <div className="max-w-md mx-auto w-full">
            ...
            <div className="space-y-3 mb-8">
              <button
                  ...
                  disabled={anyLoading}
                  className="w-full bg-white dark:bg-earth-800 border-2 border-earth-100 dark:border-earth-700 text-slate-700 dark:text-earth-200 font-black py-4 rounded-2xl hover:bg-earth-50 dark:hover:bg-earth-700 transition-all flex items-center justify-center gap-4 shadow-sm active:scale-95 disabled:opacity-50 text-[10px] uppercase tracking-widest"
              >
              ...
              <button
                  ...
                  disabled={anyLoading}
                  className="w-full bg-slate-900 dark:bg-slate-800 border-2 border-slate-800 dark:border-slate-700 text-white font-black py-4 rounded-2xl hover:bg-slate-800 dark:hover:bg-slate-700 transition-all flex items-center justify-center gap-4 shadow-sm active:scale-95 disabled:opacity-50 text-[10px] uppercase tracking-widest"
              >
                  ...
                  {isSignUp ? "Register with GitHub" : "Login with GitHub"}
              </button>
               <button
                  type="button"
              ...
              >
                  ...
                  {isSignUp ? "Register with Microsoft" : "Login with Microsoft"}
              </button>
            </div>
            ...
            <div className="mt-10 text-center">
                <p className="text-[10px] font-bold text-earth-400 uppercase tracking-widest">
                 ...
                 <p className="mt-6 text-center text-xs text-earth-400 font-medium">
                    Or return to the
                    <button 
                        onClick={() => onNavigate(View.HOME)}
                        className="ml-1 font-bold text-agro-700 dark:text-agro-400 hover:underline"
                    >
                        Homepage
                    </button>.
                </p>
