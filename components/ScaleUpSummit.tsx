...
import { Ticket, Users, Mic, Laptop, Coffee, Rocket, ArrowRight, ArrowLeft } from 'lucide-react';
import { WorkshopSection } from './scaleup/WorkshopSection';
import { RegistrationForm } from './scaleup/RegistrationForm';
import { View } from '../types';

interface ScaleUpSummitProps {
    onNavigate?: (view: View) => void;
}

export const ScaleUpSummit: React.FC<ScaleUpSummitProps> = ({ onNavigate }) => {
    ...
    <div className="max-w-7xl mx-auto px-6 py-12 animate-in fade-in duration-700">
      <div className="max-w-7xl mx-auto mb-8 flex justify-between items-center">
        <button 
          ...
          className="flex items-center gap-2 text-gray-500 hover:text-indigo-600 transition-colors text-sm font-bold uppercase tracking-widest"
        >
                    ...
                    <div className="lg:col-span-2 space-y-12">
                        ...
                        <div className="bg-white dark:bg-earth-900 border border-earth-100 dark:border-earth-800 p-12 rounded-[3rem] shadow-sm">
                            ...
                            <div className="grid md:grid-cols-3 gap-8">
                                {[
                                    { title: 'Global Access', desc: 'Direct link to EnvirosAgro hubs.', icon: <Users className="text-indigo-500" /> },
                                    { title: 'Deep Data', desc: 'Exclusive access to EA Thrust logs.', icon: <Laptop className="text-blue-500" /> },
                                    { title: 'Market Scale', desc: 'Industrialized restoration models.', icon: <Rocket className="text-indigo-600" /> }
                                ].map((why, i) => (
                                    <div key={i} className="text-center">
                                        <div className="w-12 h-12 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 rounded-xl flex items-center justify-center mx-auto mb-4">{why.icon}</div>
                    ...
                    <div className="space-y-8">
                        <div className="bg-indigo-900 dark:bg-indigo-950 rounded-[2.5rem] p-10 text-white shadow-2xl relative overflow-hidden group border border-indigo-500/30">
                            ...
                            <button 
                                ...
                                className="w-full py-5 bg-white text-indigo-900 rounded-2xl font-black text-xs uppercase tracking-[0.3em] hover:bg-indigo-50 transition-all shadow-xl active:scale-95 flex items-center justify-center gap-3"
                            >
