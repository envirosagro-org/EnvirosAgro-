...
/* Added missing icon imports ArrowUpRight and CheckCircle2 from lucide-react */
import { 
...
/**
 * Utility to crop and resize image via Canvas
 */
const getCroppedImg = (imageSrc: string, pixelCrop: { x: number, y: number, width: number, height: number }): Promise<string> => {
  return new Promise((resolve, reject) => {
    ...
    image.onload = () => {
      ...
      // Set target size (standard 400x400 for profiles)
      canvas.width = 400;
      canvas.height = 400;
...
export const UserProfile: React.FC<UserProfileProps> = ({ user, onUpdateUser }) => {
  ...
  const finalizeImage = async () => {
    ...
    try {
        // Simplified crop: take the center square
        const img = new Image();
        img.src = tempImage;
                    ...
                    <div className="w-full space-y-6">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <button 
                                ...
                                className="bg-earth-50 dark:bg-earth-800 text-earth-700 dark:text-earth-300 py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest border border-earth-100 dark:border-earth-700 flex items-center justify-center gap-2 hover:bg-earth-100 transition-all"
                            >
                            ...
                            <button 
                                ...
                                className="bg-agro-600 hover:bg-agro-700 text-white py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-xl flex items-center justify-center gap-2 transition-all disabled:opacity-50"
                            >
             ...
             <div className="w-full bg-gradient-to-br from-agro-800 to-agro-950 rounded-[2rem] overflow-hidden shadow-2xl relative text-white aspect-[1.58/1] group">
                ...
                <div className="relative z-10 px-6 mt-2 flex gap-6">
                      <div className="relative group/avatar">
                          ...
                          <button 
                            onClick={() => fileInputRef.current?.click()}
                            className="absolute -bottom-2 -right-2 bg-agro-600 text-white p-2.5 rounded-2xl shadow-xl hover:bg-agro-700 transition-all active:scale-90 border-4 border-white dark:border-earth-900 opacity-0 group-hover/avatar:opacity-100"
                          >
                            <Camera size={16} />
                          </button>
                      </div>
           ...
           <div className="bg-white dark:bg-earth-900 p-10 rounded-[3rem] shadow-sm border border-earth-100 dark:border-earth-800 relative transition-all">
                 ...
                 {!isEditing ? (
                    <>
                        <button 
                            ...
                            className="flex items-center gap-2 text-earth-50 hover:text-agro-600 font-black text-[10px] uppercase tracking-widest transition-all"
                        >
                    ...
                    <button 
                        ...
                        className="flex items-center gap-2 text-earth-400 hover:text-red-500 font-black text-[10px] uppercase tracking-widest transition-all"
                    >
                        <X size={16} /> Cancel
                    </button>
                 )}
              ...
              {isEditing ? (
                 <form onSubmit={handleSave} className="space-y-8">
                        ...
                        <div className="space-y-2">
                           ...
                           <div className="relative">
                               ...
                               <input 
                                  ...
                                  className="w-full bg-earth-50 dark:bg-earth-800 border-2 border-transparent focus:border-agro-500 rounded-2xl pl-12 pr-4 py-4 font-bold text-earth-900 dark:text-white transition-all outline-none"
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="flex items-center gap-8 bg-earth-50 dark:bg-earth-800 p-8 rounded-3xl border border-earth-100 dark:border-earth-700">
                           <div className="relative group cursor-pointer" onClick={() => fileInputRef.current?.click()}>
                                <div className="w-24 h-24 bg-white dark:bg-earth-900 rounded-full overflow-hidden border-4 border-white dark:border-earth-700 shadow-xl group-hover:brightness-75 transition-all">
                                    {formData.avatar ? (
                                        ...
                                        <div className="w-full h-full flex items-center justify-center text-earth-300"><ImageIcon size={40}/></div>
                                    )}
                                </div>
                           ...
                           <div className="flex-1">
                               ...
                               <p className="text-xs text-earth-500 dark:text-earth-400 mb-4 leading-relaxed">Images are standardized for network identification. JPG, PNG or WEBP accepted.</p>
                               <input 
                                  type="file" 
                                  ref={fileInputRef}
                                  className="hidden" 
                               ...
                               <button 
                                  type="button" 
                                  onClick={() => fileInputRef.current?.click()}
                                  className="text-[10px] font-black uppercase tracking-widest text-agro-600 hover:text-agro-700 transition-colors flex items-center gap-2"
                               >
                    ...
                    <div className="space-y-2">
                       ...
                       <textarea 
                          ...
                          className="w-full bg-earth-50 dark:bg-earth-800 border-2 border-transparent focus:border-agro-500 rounded-[2rem] px-8 py-6 font-medium text-earth-900 dark:text-white transition-all outline-none resize-none leading-relaxed"
                          placeholder="Document your agricultural journey and localized expertise..."
                       />
                    </div>
                    ...
                    <div className="flex gap-4 pt-4">
                        <button 
                            type="button" 
                            onClick={handleCancel}
                            className="flex-1 py-5 bg-white dark:bg-transparent border-2 border-earth-100 dark:border-earth-700 text-earth-50 rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all hover:bg-earth-50"
                        >
                            Cancel Changes
                        </button>
                        <button 
                            type="submit" 
                            className="flex-1 bg-agro-900 text-white font-black py-5 rounded-2xl shadow-2xl shadow-agro-900/30 hover:bg-agro-950 transition-all flex items-center justify-center gap-3 text-[10px] uppercase tracking-widest active:scale-95"
                        >
                            <Save size={18} /> Update Node Info
                        </button>
                    </div>
                 </form>
              ) : (
                 <>
                    ...
                    <div className="relative">
                        <Quote size={48} className="absolute -top-6 -left-6 text-earth-50 dark:text-earth-800/40 pointer-events-none" />
